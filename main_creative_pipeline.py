...
async def run_stage_2(master_payload: dict) -> dict:
    ...
    if not drafts:
        raise RuntimeError("[Stage2] Khong co drafts nao duoc tao ra.")

    # 3. Map voice cho nhan vat
    voice_mapper = VoiceMapper()
    all_characters = master_payload.get("protagonists", []) + master_payload.get("supporting_cast", [])
    char_names = [c.get("name") for c in all_characters if isinstance(c, dict) and c.get("name")]
    ...
    if decision.startswith("APPROVE_"):
        ...
        logger.info(f"[Stage2] Showrunner DA CHON BAN SO {draft_idx + 1}")
        ...
        logger.info("[Stage2] Showrunner yeu cau viet lai cuc doan")
        post_handler.handle_rewrite_extreme(master_payload)
        raise RuntimeError("Showrunner requested extreme rewrite.")
    else:
        logger.info("[Stage2] Showrunner TU CHOI")
        raise RuntimeError("Showrunner rejected the scripts.")
